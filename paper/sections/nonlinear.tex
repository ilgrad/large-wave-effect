...
\emph{The virial obstruction, located precisely.} A second-moment (Morawetz-type) attempt pinpoints the
wall. On the line---the ring for $t\lesssim N/5$, where aliasing is negligible---the spread
$V(t)=\sum_jj^2|a_j|^2$ satisfies $\ddot V(0)=4P$ \emph{independently of $\gamma$} (the cubic term vanishes
at the single-site seed), and numerically $\ddot V>0$ throughout the window, decaying to $0$ as $\gamma
P\to4$: the virial is convex, $V\to\infty$, which \emph{supports} dispersal. But the convexity constant is
the asymptotic $\ddot V\to2\langle v^2\rangle$ with group velocity $v_k=2\sin k$, and $\langle
v^2\rangle=2-2\langle\cos2k\rangle$ is a \emph{second} spectral moment. Energy conservation fixes only the
first, $\langle\cos k\rangle_\infty=\gamma P/4$ (equivalent to $\langle\LN u,u\rangle\to2P-\tfrac\gamma2P^2$
above), which is $<1$ for $\gamma P<4$ but does \emph{not} force $\langle v^2\rangle>0$: a momentum
distribution split bimodally between $k=0$ and $k=\pi$ has $\langle\cos k\rangle=\gamma P/4$ yet $\langle
v^2\rangle=0$. So the conserved quantities alone cannot lower-bound the virial convexity---the dynamics
does avoid the staggered ($k=\pi$) trap (numerically $\langle v^2\rangle>0$), but conservation does not see
it. This is exactly where Morawetz/Strichartz arguments stall, and it identifies the missing ingredient: a
rigorous bound on the staggered momentum content $\langle\cos2k\rangle$ (\texttt{verify\_virial\_window.py}).
