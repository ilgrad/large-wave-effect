...
equation~\eqref{eq:dnls} is the natural nonlinear continuation of the linear \emph{Schr\"odinger} large wave
$B_N$ of Section~\ref{sec:schr} (the first-order $\ell^1\!\to\!\ell^\infty$ amplification), not of the
second-order mechanical wave $A_N$ of Part~I---the two are distinct large-wave regimes on the same $\LN$.
Proposition~\ref{prop:dnls-persist} accordingly persists the \emph{Schr\"odinger} peak, not the bead-chain one.
...
bandwidth: the spreading excitation must outrun the entire band $[0,4]$, not merely its edge. This
single-site barrier at $A_*^2=2(\sigma+1)^{1/\sigma}=4$ (cubic $\sigma=1$) was identified by
Kevrekidis--Espinola-Rocha--Drossinos--Stefanov~\cite{KERDS08} as a \emph{sufficient} trapping condition
($H_{\mathrm{ss}}<0$); we make that side a theorem with the sharp constant and show it is \emph{energy-sharp}
(below). One side of the dynamical threshold is thus settled, with the constant exactly the bandwidth $4$.
(The \emph{static} excitation threshold of~\cite{Weinstein99} is a different object: a positive minimal
power for a ground state exists only when $\sigma\ge2/d$, so in $1$D cubic there is none---localized modes
exist at arbitrarily small power, while the dynamical launch barrier $4$ persists.)
...
\emph{The virial obstruction, located precisely.} A second-moment (Morawetz-type) attempt isolates the
wall to a single estimate. On the line---the ring for $t\lesssim N/5$, where aliasing is negligible---the
spread $V(t)=\sum_jj^2|a_j|^2$ obeys the \emph{exact} identity (verified to machine precision along the
flow, \texttt{verify\_virial\_window.py})
\begin{equation}\label{eq:virial}
  \ddot V\;=\;2\langle L_2a,a\rangle\;-\;2\gamma\sum_n(2n+1)\bigl(|a_n|^2-|a_{n+1}|^2\bigr)\,
  \mathrm{Re}(\bar a_na_{n+1}),
\end{equation}
where $L_2$ is the next-nearest-neighbour Laplacian (symbol $2-2\cos2k=4\sin^2k$), so
$2\langle L_2a,a\rangle=2\langle v^2\rangle P\ge0$ ($v_k=2\sin k$ the group velocity) is a manifestly
non-negative \emph{dispersive} term and the position-weighted sum is the nonlinear correction. At the
single-site seed the correction vanishes, giving $\ddot V(0)=4P$ independently of $\gamma$; numerically
$\ddot V>0$ throughout the window, decaying to $0$ as $\gamma P\to4$, so the virial is convex, $V\to\infty$,
which \emph{supports} dispersal. Closing the argument needs the correction bounded by
$(1-\epsilon)\,2\langle L_2a,a\rangle$; numerically it is small (a few percent of the dispersive term) and
degenerates to exactly $-2\langle L_2a,a\rangle$ as $\gamma P\to4$, but its position weight $(2n+1)$ is not
controlled by the conserved mass, energy, and momentum. Indeed energy fixes only the \emph{first} spectral
moment, $\langle\cos k\rangle_\infty=\gamma P/4$ (equivalent to $\langle\LN u,u\rangle\to2P-\tfrac\gamma2P^2$
above), $<1$ for $\gamma P<4$ but not forcing $\langle v^2\rangle=2-2\langle\cos2k\rangle>0$: a bimodal
$k=0/\pi$ momentum split has $\langle\cos k\rangle=\gamma P/4$ yet $\langle v^2\rangle=0$. So the dynamics
avoids the staggered ($k=\pi$) trap (numerically $\langle v^2\rangle>0$) but the conserved quantities do not
see it. This is exactly where Morawetz/Strichartz arguments stall: the discrete Strichartz estimates of
Stefanov--Kevrekidis~\cite{StefanovKevrekidis05} (built on the sharp $t^{-1/3}$ decay~\cite{Landau00,EKT15})
close small-data scattering only for $\sigma\ge3/d$---in $1$D, power $\ge7$, not the cubic
here---so they provably cannot reach the window, and the continuum interaction-Morawetz/concentration
machinery consumes the Galilean invariance the lattice lacks. Equation~\eqref{eq:virial} thus reduces the
open problem to a single estimate---a bound on the position-weighted nonlinear term, i.e.\ on the staggered
momentum content $\langle\cos2k\rangle$.

\emph{An equivalent, position-weight-free form.} The dispersive term is set by the staggered momentum
$\langle L_2a,a\rangle=2P(1-\langle\cos2k\rangle)$, $\langle\cos2k\rangle=P^{-1}\mathrm{Re}\sum_n\bar a_na_{n+2}$,
and---because the linear flow preserves every momentum moment $|\hat a_k|^2$---its evolution is purely
nonlinear and carries \emph{no} position weight:
\begin{equation}\label{eq:stagger}
  \frac{d}{dt}\,\mathrm{Re}\!\sum_n\bar a_na_{n+2}
   =-\gamma\sum_n\bigl(|a_n|^2-|a_{n+2}|^2\bigr)\mathrm{Im}(\bar a_na_{n+2}),\qquad
  \Bigl|\frac{d}{dt}\,\mathrm{Re}\!\sum_n\bar a_na_{n+2}\Bigr|\le2\gamma\|a\|_4^4
\end{equation}
(the identity proven symbolically; the bound by Cauchy--Schwarz; \texttt{verify\_stagger\_morawetz.py}). So
the whole question rests on the \emph{focusing interaction-Morawetz} spacetime norm
$\gamma\int_0^\infty\|a\|_4^4\,dt$---the standard scattering criterion---which is numerically bounded for
$\gamma P<4$ and divergent above (matching the sharp threshold $4$); its focusing-sign obstruction is exactly
the barrier~\cite{StefanovKevrekidis05} cannot cross at the cubic power. The integrable Ablowitz--Ladik
cousin, whose hierarchy \emph{conserves} a staggered-momentum functional, motivates~\eqref{eq:stagger}; but
that functional \emph{drifts} under the non-integrable flow~\eqref{eq:dnls}, the $O(1)$ Ablowitz--Ladik gap
made quantitative.

\begin{lemma}[the leading staggered-momentum drift vanishes]\label{lem:stagger-leading}
Along the \emph{linear} flow $\mathrm i\dot a=-\LN a$ from the single-site seed $a_j(0)=\sqrt P\,\delta_{j0}$
(on the line, i.e.\ the ring for $t\lesssim N/5$), the distance-$2$ current vanishes identically,
$\operatorname{Im}\bigl(\bar a_n(t)\,a_{n+2}(t)\bigr)=0$ for all $n,t$; hence the right-hand side
of~\eqref{eq:stagger} is zero on the linear flow.
\end{lemma}
\begin{proof}
By Jacobi--Anger the linear evolution of the seed is $a_n(t)=\sqrt P\,\eu^{2\mathrm i t}(-\mathrm i)^n J_n(2t)$
with $J_n$ real; it solves $\mathrm i\dot a=-\LN a$ by the Bessel recurrence $J_n'=\tfrac12(J_{n-1}-J_{n+1})$.
Hence $\bar a_na_{n+2}=P\,(\mathrm i)^n(-\mathrm i)^{n+2}J_n(2t)J_{n+2}(2t)=-P\,J_n(2t)J_{n+2}(2t)\in\R$, since
$(\mathrm i)^n(-\mathrm i)^{n+2}=[\mathrm i(-\mathrm i)]^n(-\mathrm i)^2=-1$: the lattice phase $(-\mathrm i)^n$
holds next-nearest sites in phase, so $a_{n+2}$ is a \emph{real} multiple of $a_n$ and
$\operatorname{Im}(\bar a_na_{n+2})=0$. The closed form and the phase identity are checked symbolically
(\texttt{verify\_staggered\_current.mac}, Maxima), the ``real multiple $\Rightarrow$ zero current'' step is
machine-checked in Rocq (\texttt{formal/rocq/StaggeredCurrent.v}), and the pointwise vanishing is confirmed
on the discrete flow, where the distance-$1$ current is $O(1)$ by contrast
(\texttt{verify\_staggered\_current.py}). \end{proof}

\noindent Since the right-hand side of~\eqref{eq:stagger} is $O(\gamma)$ and vanishes on the linear flow
(Lemma~\ref{lem:stagger-leading}), the staggered momentum drifts only at $O(\gamma^2)$: numerically
$\sup_t|\Delta\!\operatorname{Re}\!\sum_n\bar a_na_{n+2}|\sim(\gamma P)^2$ (exponent $2.08$, uniform in $N$
over $N=1024$--$4096$), with $\inf_t\langle v^2\rangle\simeq0.49>0$ across the window
(\texttt{verify\_modified\_energy\_window.py}). The lemma removes the leading order of the obstruction;
closing the window still needs an $N$-uniform bound on the $O(\gamma^2)$ remainder and the nonperturbative
approach to $\gamma P=4$.
...
\begin{proof}
Grillakis--Shatah--Strauss gives orbital stability once three hypotheses hold: (i) the slope
$\mathrm dP/\mathrm d\Omega>0$; (ii) $L_-\ge0$ with $\ker L_-=\operatorname{span}\phi$; (iii) $n(L_+)=1$, a
single negative direction. We establish them.
\emph{(i)} The anti-continuum expansion (one site of weight $\sqrt P$ gives $\Omega=\gamma P-2+O(1/P)$) yields
$P(\Omega)=\Omega/\gamma+O(1)$, so $\mathrm dP/\mathrm d\Omega=1/\gamma+O(1/P)>0$.
\emph{(ii)} The site-centred profile is \emph{positive}, $\phi_\Omega>0$ (anti-continuum construction with a
positive seed, preserved under continuation), and $L_-\phi=0$ by the stationary equation
$\LN\phi+\gamma\phi^3=\Omega\phi$. A strictly positive vector in the kernel of the discrete Schr\"odinger
operator $L_-=\LN+\Omega-\gamma\phi^2$ is, by Perron--Frobenius on the finite ring, its ground state; hence
$L_-\ge0$ with simple kernel $\operatorname{span}\phi$.
\emph{(iii)} $\langle L_+\phi,\phi\rangle=\langle L_-\phi,\phi\rangle-2\gamma\|\phi\|_4^4=-2\gamma\|\phi\|_4^4<0$,
so $n(L_+)\ge1$; in the anti-continuum limit $L_+=\LN+\Omega-3\gamma\phi^2$ has exactly one negative
eigenvalue (the deep well $-3\gamma\phi^2$ at the excited site against $\LN+\Omega>0$ elsewhere), and this
persists along the branch since no eigenvalue of $L_+$ crosses $0$ on the tested range, so $n(L_+)=1$.
With (i)--(iii), GSS yields orbital stability of the site-centred wave. \emph{Both} branches are moreover
\emph{certified in exact rational arithmetic} (\texttt{verify\_dnls\_breather\_cap.py}): a Newton--Kantorovich
test promotes a rational profile $\tilde\phi$ to a true breather (here $\|\phi-\tilde\phi\|_2\le4\times10^{-9}$),
exact $\mathrm{LDL}^\top$ inertia (Sylvester's law) at $\pm I$ pins the Morse index with the spectrum a distance
$\ge1$ from $0$, and a congruence $Q^\top(L_--I)Q\succ0$ on $\tilde\phi^\perp$ gives $\lambda_2(L_-)\ge1$; a
Weyl bound ($\delta\le5\times10^{-8}\ll1$) carries both signatures to the true $\phi$. This yields $n(L_+)=1$
for the site-centred ring $N=5,\Omega=2$ (GSS index $0$, stable) and $n(L_+)=2$ for the bond-centred ring
$N=6,\Omega=2$ (GSS index $n(L_+)-1=1$, odd, hence orbitally \emph{unstable})---upgrading both verdicts from
numerical to computer-assisted. Over the full branch \texttt{verify\_dnls\_breather\_stability.py} tracks the
spectrum ($\max|\mathrm{Re}\,\lambda|\sim10^{-7}$ site; a real pair $\in[3.9,8.9]$ bond) on
