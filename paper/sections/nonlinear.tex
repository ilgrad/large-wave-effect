...
does not grow without bound but saturates into an Akhmediev breather---the periodic avatar of the
rogue spike.

\emph{From thin tails to heavy tails.} The statistical signature is the sharpest contrast with the
linear theory. Seeded by a noisy plane wave, \eqref{eq:dnls} on the ring spawns localized bursts
exceeding twice the root-mean-square amplitude---the oceanographic rogue threshold---and the amplitude
...
site detunes out of the linear band and freezes into a stationary, exponentially localized \emph{discrete
breather} $z_j(t)=\phi_j\eu^{-\mathrm i\Omega t}$, whose real profile solves
...
stability of these breathers (MacKay--Aubry theory) and the precise dynamical threshold are the rigorous
sequel left open here.
...
Right: above threshold the excitation is a stationary discrete breather---a localized column in
...
stationary, exponentially localized \emph{two-dimensional discrete breather} once the on-site nonlinear
...
This multidimensional, strongly nonlinear regime is the home of Zakharov's wider program: the
Zakharov--Kuznetsov equation~\cite{ZakharovKuznetsov} is the multidimensional generalization of the
long-wave (KdV-type) limit of \eqref{eq:fput}, and the statistics of the thermalizing lattice are the
subject of the weak-turbulence (Kolmogorov--Zakharov) theory of wave spectra~\cite{ZakharovLvovFalkovich},
whose underlying wave kinetic equation has only recently been derived rigorously~\cite{DengHani}. These
...
stationary localized hump above the $2$D band top ($\gamma\approx8$). Nonlinearity restores a large
