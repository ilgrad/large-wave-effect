...
clean reason: the \emph{excess} $A_N-L_{\mathrm{pre}}$ stays in $[0.05,0.10]$ across every tested $N$
($\omega(m)\le3$, prefix dimension up to $48$), so numerically
\[
...
time average $\approx0.79\sqrt N$ and never exceeds the $t=N/2$ value: the $\sin^2$ dispersion admits no flat
(Gauss-sum) revival, even at structured Talbot times $t=\pi p/q$. This is consistent with the
