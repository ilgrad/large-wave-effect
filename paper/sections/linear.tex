...
\begin{proof}
A nontrivial relation $\sum_{r=1}^K k_r\sin(\pi r/N)=0$ with $k_K\ne0$ ($K$ its largest index) gives, with
$\zeta=\zeta_{2N}$ and $P(x)=\sum_{r=1}^K k_r x^r$,
\[
...
violates both mod-$4$ branches. Only the cyclotomic input---that the constructed vector is a genuine
relation---is supplied from outside (exactly, by \textsc{GAP}/\textsc{PARI} and
\texttt{verify\_classification\_proof.py}).
