...
growth of $U_N$ is exactly the integrable singularity of $f^{-1/2}$ at $\theta=0$. (Szeg\H{o}'s theorem
...
$\sin^2(t\omega_r)=1$ simultaneously---rational independence ($N$ prime, $2^m$, $2p$). Finally
...
node over the energy sphere $E(u_0,v_0)\le E_0$ is a constrained quadratic problem solved by Lagrange
multipliers,
\[
...
unconditional; the other two equal the listed ceilings on the $\Q$-independent classes ($N$ prime,
$2^m$, $2p$) and are bounded by them in general. The large wave is the $\ell^1\!\to\!\ell^\infty$
corner.}\label{tab:opnorm}
\end{table}
...
\]
the ceiling attained with no gap (in contrast to the composite $N=12$ below;
...
\emph{iff $N+1$ is prime or a power of two}---the exact analogue of Theorems~\ref{thm:prime}--\ref{thm:two}
...
\end{equation}
a finite-dimensional trigonometric maximization solved exactly (\texttt{verify\_exact\_AN.py},
validated on primes where it returns $A_N=U_N$). This makes the composite case computable for each $N$:
...
$A_N$ is an algebraic number: substituting $x_r=\cos\varphi_r$, $y_r=\sin\varphi_r$ with
$x_r^2+y_r^2=1$ turns \eqref{eq:subtorus} into the maximum of a polynomial over a compact semialgebraic
set, whose value is algebraic. For $N=6$ the relation lattice is one-dimensional and
\[
...
frequencies $\omega_r=2\sin(\pi r/N)$, $1\le r\le m=\lfloor N/2\rfloor$. Then
\[
  A_N=U_N\quad\Longleftrightarrow\quad \sum_{r}k_r\equiv0\ (\mathrm{mod}\ 4)\quad\text{for every }k\in\Lambda .
\]
\end{theorem}

\begin{proof}
Always $A_N\le U_N$, with $U_N=\sum_r a_{r0}$ the node-$0$ ceiling and $|a_{rj}|\le a_{r0}$, so no node can
exceed $U_N$ and the question is whether node $0$ attains it. There the coefficients $a_{r0}>0$, so the
value $U_N$ is reached iff all $\sin(\omega_r t)$ can be driven to $1$ simultaneously, i.e.\ the target
phase $\varphi_\ast=(\tfrac\pi2,\dots,\tfrac\pi2)$ lies in the orbit closure $\mathbb{T}_N$. By
Kronecker, $\varphi_\ast\in\mathbb{T}_N$ iff $\langle k,\varphi_\ast\rangle\equiv0\ (2\pi)$ for every
$k\in\Lambda$; and $\langle k,\varphi_\ast\rangle=\tfrac\pi2\sum_r k_r\equiv0\ (2\pi)$ iff
$\sum_r k_r\equiv0\ (4)$.
\end{proof}
...
relation with coefficient-sum $\not\equiv0\ (\mathrm{mod}\ 4)$, hence $A_N<U_N$
(\texttt{verify\_relation\_lattices.py}).}\label{tab:lattices}
...
\begin{corollary}[the saturating set is thin]\label{cor:density}
The two proven saturating families, the primes and the powers of two, both have natural density zero.
The criterion finds no further saturating $N\le160$, so there $\{N:A_N=U_N\}=\{N\text{ prime}\}\cup\{2^m\}$
and an asymptotically full set of $N$ satisfies $A_N<U_N$. Whether any composite $N$ saturates at all is
decided, for each $N$, by Theorem~\ref{thm:ceiling}.
\end{corollary}
...
\begin{proof}
The prefix $\{\omega_1,\dots,\omega_{p-1}\}$ equals the spectrum of the Dirichlet segment on $p-1$
nodes, which is $\Q$-independent because $(p-1)+1=p$ is prime \eqref{eq:segrank}; the only remaining
frequency is the rational $\omega_p=2$, of weight $\tfrac1{2N}$. With the degeneracy
...
once---a simultaneous Diophantine approximation of the phases to $\tfrac\pi2$. By Dirichlet, aligning
$m$ phases to tolerance $\delta$ costs time up to $\sim\delta^{-m}$, so the recurrence time
\[
  T_\varepsilon(N)=\inf\{T:A_N(T)\ge(1-\varepsilon)U_N\}
\]
grows at least exponentially in $N$.
Numerically, for primes $\log T_{0.2}(N)$ grows by $\approx0.63$ per unit $N$
...
$c_r=v_r(0)^2/\omega_r>0$ and incoherent ceiling $C=\sum_r c_r$. Generic masses destroy every rational
relation almost surely, so the obstruction of Theorem~\ref{thm:ceiling} disappears and \emph{every} $N$
...
\caption{Reachability of the ceiling. Left: the recurrence time $T_{0.2}(N)$ to reach $0.8\,U_N$ grows
exponentially in $N$ (primes, log scale). Right: at a fixed horizon $T$ the finite-time efficiency
...
$B_N\ge c\sqrt N$ with an absolute constant $c>0$ (numerically $c\gtrsim0.63$). With
Theorem~\ref{thm:schr}(2), $B_N=\Theta(\sqrt N)$.
\end{theorem}
...
$\sum_n|J_n(N/4)|\sim c_0\sqrt{N/4}$. Therefore $B_N\ge\sum_j|K_N(j,N/8)|\ge c\sqrt N$; the upper bound
is Theorem~\ref{thm:schr}(2).
\end{proof}
...
periodization, the negligible aliasing at $t=N/8$, the $\sqrt N$ growth with $c\gtrsim0.63$, and the
...
\begin{table}[ht]\centering
\caption{The constant in $B_N=\Theta(\sqrt N)$. A refined $t$-scan (uniform grid $+$ Talbot times
$t=\pi a/N$) gives $B_N/\sqrt N$; the proven Bessel point $t=N/8$ gives the lower row. The empirical
$\sup_N B_N/\sqrt N\approx0.96$ lies inside the rigorous band $(0.63,1]$, and the exact limit is open
...
The recurrence structure of $\eu^{-\mathrm i t\LN}$ is arithmetic. Since $2\cos(2\pi r/N)$ is rational
only for $N\in\{1,2,3,4,6\}$ (Niven's theorem~\cite{Niven}), the tight-binding spectrum is generically
...
arithmetic: at the Talbot times $t=2\pi s/q$ the propagator kernel is a quadratic Gauss sum, and for
...
the \emph{plus}-eigenspace---has full rank $\lfloor(N-1)/2\rfloor$ exactly for $N$ prime or $N=2^m$, and
fails for composite $N$ (\texttt{verify\_filimonov\_schrodinger.py}). So Filimonov's sufficient condition
$N=2^m$ in fact extends to prime $N$, while a composite $N$ leaves a gap $\sup_t|z_j|<d_j^N$.
...
$B_N$ & ring, $\ell^\infty\!\to\!\ell^\infty$ Schr\"odinger & $\Theta(\sqrt N)$, const $\in[0.63,1]$ & all $N$\\
$d_j^N$ & segment, $\sup_t|z_j(\cdot)|$ Schr\"odinger & $\tfrac4{\pi^2}\ln j$ & prime, $2^m$\\
...
supremum (the true maximum is attained only at near-recurrence times). The present analysis thus
...
The wave $A_N$ is the exact orbit-closure value of Section~\ref{sec:exact}; it equals the ceiling
...
$\rho(\omega)=2/(\pi\sqrt{4-\omega^2})$ on $[0,2]$. Both fit to Kolmogorov--Smirnov distance $O(N^{-1/2})$.
...
\medskip\noindent\textbf{Spectral dimension.} The threshold is not metric but \emph{spectral}, and the
same argument applies to any finite graph. The relevant ceiling is
$U_G=|V|^{-1}\sum_{\lambda_r>0}\lambda_r^{-1/2}$ ($\lambda_r$ the Laplacian spectrum, $\omega_r=\sqrt{\lambda_r}$).
...
converges at the origin iff $d_s>1$. Hence the large wave occurs precisely at the critical spectral
dimension $d_s=1$ (logarithmic growth), is absent for $d_s>1$ (bounded ceiling), and would be power-law
for $d_s<1$; the $d$-torus is the case $d_s=d$. The criterion is geometric rather than metric: any
\emph{quasi-one-dimensional} lattice---a ladder $C_N\times C_2$ or a tube $C_N\times C_w$ of bounded
width $w$---still has $d_s=1$ and retains the logarithmic large wave, now with reduced prefactor
...
for the $d_s=2$ square torus. The large wave is a critical-dimension ($d_s=1$) phenomenon.}\label{fig:specdim}
\end{figure}
