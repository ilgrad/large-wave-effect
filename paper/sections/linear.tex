...
$\Q$ (the prime analogue of Theorem~\ref{thm:prime}; exact integer rank $=p-1$,
\texttt{verify\_2p\_family.py}), so the sole $\Q$-dependent frequency is the rational $\omega_p=2$, of
...
\]
A matching \emph{lower} bound (that one must wait at least exponentially long) does \emph{not} follow
...
$c_0=(2/\pi)^{3/2}B(\tfrac12,\tfrac34)$, i.e.\ $\liminf_N B_N/\sqrt N\ge0.6086$, and one may take $c=0.60$
for all $N$. With Theorem~\ref{thm:schr}(2), $B_N=\Theta(\sqrt N)$.
\end{theorem}
...
$\liminf_N B_N/\sqrt N\ge\lim_N\sum_j|K_N(j,N/8)|/\sqrt N=\tfrac12 c_0$, so $B_N\ge0.60\sqrt N$ for all $N$
...
$\sup_N B_N/\sqrt N\approx0.96$ lies inside the rigorous band $[\tfrac12 c_0,1]\approx[0.6086,1]$, and the
exact limit is open
