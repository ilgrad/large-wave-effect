...
Section~\ref{sec:exact}, where $A_N/\ln N$ stays in $[0.28,0.34]$ across the tested range (the composite,
non-saturating values lying in $[0.28,0.31]$, the saturating prime/$2^m$ values reaching $U_N/\ln N$ up to
$0.34$)
...
by the boundary (the ring uses modulus $2N$, the segment $4(N+1)$). The $\Q$-independent low-mode
...
lies slightly below, and conjecturally $A_N=\Theta(\ln N)$ for every $N$ (Conjecture~\ref{conj:order}).}\label{fig:ring}
\end{figure}
...
(equivalently, a sharp lower bound on the subtorus maximum). \emph{Soft bounds do not suffice}: the
time-averaged second moment gives only $\sup_t|G_N(0,t)|\ge\mathrm{RMS}_t\to1/\sqrt{12}$, and the single
slowest mode only $b_1\to1/\pi$, both $O(1)$; the $\ln N$ must come from the simultaneous near-alignment of
$\Theta(\ln N)$ modes. A certified value for each $N$ is available from the Lasserre moment--SOS hierarchy;
the analytic obstruction is the control of the rational relations among $\{2\sin(\pi r/N)\}$ (vanishing
sums of roots of unity~\cite{CJ})---a question in the arithmetic of cyclotomic fields.
