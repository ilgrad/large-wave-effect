...
$2t/N=1$---i.e.\ at $t=N/2$---with value $\beta_{\mathrm{odd}}$, \emph{proved analytically} on $87\%$ of the
...
residual $[0.937,1]$ the profile is real-analytic (the substitution removes the band-edge singularities) and
strictly increasing: a Lipschitz--grid bound gives $\inf F'\ge\min_iF'(s_i)-\max_i|F''|\,\tfrac\delta2\ge
0.20>0$ ($\delta=0.008$, $\min_iF'(s_i)=0.205$, $\max_i|F''|=0.13$), so $F\le F(1)$. Since no strict majorant
of $g$ can stay below $\beta_{\mathrm{odd}}$ at the exact maximizer $s=1$, this last monotonicity is the
residual step; crucially it is one-dimensional, free of $N$ (\texttt{verify\_bn\_profile\_max.py}).
\end{proof}
