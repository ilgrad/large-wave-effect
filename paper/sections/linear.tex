...
$g(a,b)=\mathbb E_{\Phi,\Psi}\sqrt{a\cos^2\Phi+b\cos^2\Psi}$ with $\Phi,\Psi$ independent uniform. Then
$F(\tfrac12)=c_0/\sqrt2$ and $F(1)=\beta_{\mathrm{odd}}$ \eqref{eq:beta-odd}, and
\[
...
$\min F\approx0.847$ at $s\approx0.56$ and below $\beta_{\mathrm{odd}}$. On $[\tfrac45,1]$ the scalar profile
$F$ is strictly increasing (forward differences $\ge5\cdot10^{-4}$ on a fine grid), so $F\le F(1)$ there.
Since no strict majorant of $g$ can stay below $\beta_{\mathrm{odd}}$ at the exact maximizer $s=1$, this last
monotonicity is the residual step; crucially it is a one-dimensional property of the explicit profile, free
of $N$ (\texttt{verify\_bn\_profile\_max.py}).
\end{proof}
