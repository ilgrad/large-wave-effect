...
any iterate of the logarithm. The defect is \emph{not} bounded: numerically the family limit grows with the
number $\omega(m)$ of distinct odd prime factors---$U_N-A_N\approx0.15,\,0.21$ for $\omega(m)=2,3$
(e.g.\ $N=30$ vs.\ $N=210$), tracking $\tfrac12\cdot\tfrac2\pi\ln\tfrac{m}{\varphi(m)}$---so
$U_N-A_N=\Theta(\ln\ln\ln N)$ along primorials (\texttt{verify\_defect\_growth.py}); the proven $O(\ln\ln\ln N)$
upper bound is therefore sharp in order. The lower bound is moreover \emph{rigorous} for small $N$: the
Lipschitz-grid certificate (the elementary member of the Lasserre moment--SOS hierarchy) gives a proof-level
upper bound on $A_N$, hence $U_N-A_N\ge0.049,\,0.057,\,0.030,\,0.082$ for $N=6,9,12,15$
(\texttt{verify\_defect\_certified.py}); only the asymptotic growth along primorials remains numerical (the
grid is exponential in $\tfrac12\varphi(2N)$). The exact value is algebraic per $N$
(Proposition~\ref{prop:closed}) with no universal closed form.
\end{remark}
...
locally quadratic---so $\|K_N(\cdot,t)\|_1$ is the $\ell^1$ norm of a flat-in-Fourier sequence, and by
stationary phase the two saddles $\sin(2\pi r_\ast/N)=k/2t$ govern it; the bound then reduces to the
equidistribution $\bmod\,2\pi$ of their phase difference (a Weyl sum). Numerically the full macroscopic
profile $\|K_N(\cdot,t)\|_1/\sqrt N$ peaks exactly at the $t=N/2$ value $\beta_{\mathrm{odd}}$ (odd) and is
strictly subdominant for $t>N/2$ (\texttt{verify\_bn\_parity.py}); the remaining analytic step is that Weyl
estimate.
\end{remark}
