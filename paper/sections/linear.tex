...
\noindent Beyond this collision a $100$-digit \textsc{pslq} search finds no further rational relation among
the NNN frequencies for any $5\le N\le40$; dropping the duplicate when $4\mid N$ then leaves a
$\Q$-independent set carrying the full ceiling, so on this evidence condition (B)---and with it
$A=\Theta(\ln N)$---holds throughout the NNN family. Deciding (B) for a \emph{general} $d_s=1$ lattice
remains open; the criterion isolates exactly what must be shown.
