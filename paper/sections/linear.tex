...
any iterate of the logarithm. The defect is \emph{not} bounded, and there is a clean reason: numerically the
\emph{excess} $A_N-L_{\mathrm{pre}}$ stays in $[0.05,0.10]$ across every tested $N$ ($\omega(m)\le3$, prefix
dimension up to $48$), so
\[
  U_N-A_N=(U_N-L_{\mathrm{pre}})-(A_N-L_{\mathrm{pre}})=\tfrac1\pi\ln\tfrac{m}{\varphi(m)}-O(1)\longrightarrow\infty
\]
along primorials ($\Theta(\ln\ln\ln N)$, \texttt{verify\_defect\_growth.py}); the proven $O(\ln\ln\ln N)$
upper bound is sharp in order. A rigorous \emph{growing} lower bound is thus equivalent to the
...
trigonometric Lasserre moment--SOS sharpens it (exact $U_N-A_N=0.1475$ at $N=15$) and reaches $N=105$
...
$U_{15}=0.902$, $A_{15}=0.754$, a defect $0.148$---the largest among small composites because the odd part
$m=15$ carries two prime factors (Remark~\ref{rem:defect}). The lower bound of Theorem~\ref{thm:order}
...
algebraic number (an integer-relation/\textsc{pslq} search returns nothing): $\beta_{\mathrm{odd}}$ has no
elementary closed form---it is a new constant defined by \eqref{eq:beta-odd} (\texttt{verify\_beta\_odd.py}). Evaluation at $t=N/2$ is rigorous, so $\liminf_N B_N/\sqrt N=c_0/\sqrt2$ and
$\liminf_{\text{odd }N}B_N/\sqrt N\ge\beta_{\mathrm{odd}}$, whence $\limsup_N B_N/\sqrt N\in[\beta_{\mathrm{odd}},1]$.
For the upper side, the matching bound holds on the whole window $t\le N/2$. There $2t\le N$, so only the
two Poisson copies $\ell=0,-1$ survive (the rest are super-exponentially small), $|K_N(k,t)|\le
|J_k(2t)|+|J_{N-k}(2t)|$, and the Debye envelope turns $\|K_N(\cdot,t)\|_1/\sqrt N$ into the two-copy profile
$F(2t/N)$, which is maximized at $2t/N=1$---i.e.\ at $t=N/2$---with value $\beta_{\mathrm{odd}}$
(\texttt{verify\_bn\_parity.py}). Hence $\|K_N(\cdot,t)\|_1\le(\beta_{\mathrm{odd}}+o(1))\sqrt N$ for all
...
$\lambda_r=4\sin^2(\pi r/N)$ is not. So the global sup sits at $t=N/2$ and $\limsup_N
B_N/\sqrt N=\beta_{\mathrm{odd}}$. The residual on $t>N/2$ has a clean form: $K_N(\cdot,t)$ is the inverse DFT
...
DNLS modulational-instability band (Prop.~\ref{prop:dnls-mi}) & \texttt{verify\_dnls\_mi.py}\\
