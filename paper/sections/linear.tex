...
the saturating set is \emph{exactly} the primes and the powers of two, hence of density zero.
\end{corollary}
...
intrinsic: the \emph{quadratic} chirp (Gauss--Fresnel/Talbot) is $L^1$-flat with ratio $\to1$, and ultraflat
unimodular polynomials exist~\cite{BombieriBourgain}, so $\limsup<1$ is an \emph{anti}-flatness statement
particular to the non-quadratic $\sin^2$ dispersion---no generic argument supplies it. (A single-period
