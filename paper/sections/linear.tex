...
and the same minus-eigenspace argument bounds the subtorus dimension,
\begin{equation}\label{eq:segrank}
  \dim_\Q\operatorname{span}\{\mu_1,\dots,\mu_N\}\le\tfrac12\varphi(4(N+1))
\end{equation}
(with equality to $\min\!\bigl(N,\tfrac12\varphi(4(N+1))\bigr)$ verified for $N\le60$). This upper bound
alone settles when the segment saturates.

\begin{proposition}[segment classification]\label{prop:segclass}
$A_N^{\mathrm{Dir}}=U_N^{\mathrm{Dir}}$ if and only if $N+1$ is prime or a power of two.
\end{proposition}

\begin{proof}
By the simple spectrum and Bohr, $A_N^{\mathrm{Dir}}=U_N^{\mathrm{Dir}}$ iff the $\mu_k$ are
$\Q$-independent, i.e.\ $\dim_\Q\operatorname{span}\{\mu_k\}=N$. Each $2\mathrm i\mu_k=\zeta_M^k-\zeta_M^{-k}\in V^-$
($M=4(N+1)$), so by \eqref{eq:segrank} the rank is at most $\tfrac12\varphi(M)$. Write $N+1=2^a m$ with $m$
odd; then $\tfrac12\varphi(4(N+1))=2^a\varphi(m)$, and $2^a\varphi(m)\ge N=2^am-1$ iff
$2^a\bigl(m-\varphi(m)\bigr)\le1$, i.e.\ iff $m=1$ ($N+1=2^a$) or $a=0$ with $m$ prime ($N+1$ prime). Thus
for composite $N+1$ that is not a power of two, $\dim_\Q\operatorname{span}\{\mu_k\}\le\tfrac12\varphi(4(N+1))<N$,
so the $\mu_k$ are dependent and $A_N^{\mathrm{Dir}}<U_N^{\mathrm{Dir}}$. Conversely, for $N+1=p$ prime the
$\mu_k=2\sin(\pi k/2p)$ ($1\le k\le p-1$) are $\Q$-independent (Proposition~\ref{prop:2p}), and for
$N+1=2^m$ the exponents $k$ and $2^{m+1}-k$ are distinct basis powers of $\zeta_{2^{m+2}}$ (the argument of
Theorem~\ref{thm:two}); in both cases the rank is the full $N$, so $A_N^{\mathrm{Dir}}=U_N^{\mathrm{Dir}}$
(\texttt{verify\_segment\_classification.py}). \end{proof}
...
Segment classification (Prop.~\ref{prop:segclass}) & \texttt{verify\_segment\_classification.py}\\
