...
\begin{remark}[the defect is essentially bounded]\label{rem:defect}
The same estimate bounds the deficit on the non-saturating $N$: $U_N-A_N\le2(U_N-L_{\mathrm{pre}})=
\tfrac2N\sum_{r=\frac12\varphi(2N)+1}^{\lfloor N/2\rfloor}\csc\tfrac{\pi r}N=\tfrac2\pi\ln\tfrac{N}{\varphi(2N)}+O(1)$.
Since $\varphi(2N)\ge2N/(\eu^\gamma\ln\ln2N+\cdots)$, this is $O(\ln\ln\ln N)$---so $U_N-A_N$ grows slower than
any iterate of the logarithm and is, for every practical purpose, bounded (numerically
$U_N-A_N<0.18$ for all composite $N\le60$, depending on the odd part of $N$ and converging within each
$2^a m$ family, \texttt{verify\_exact\_AN.py}). Its exact value is algebraic for each $N$
(Proposition~\ref{prop:closed}) with no apparent universal closed form.
\end{remark}
...
cancellation and $B_N/\sqrt N\to\beta_{\mathrm{odd}}\approx0.928$. Thus $\liminf_N B_N/\sqrt N=c_0/\sqrt2$
while $\limsup_N B_N/\sqrt N\ge\beta_{\mathrm{odd}}$, both inside the unitarity band $[c_0/\sqrt2,1]$; the
exact $\beta_{\mathrm{odd}}$ (a Bessel-envelope double integral) and the matching upper bound are the
residual questions.
\end{remark}
