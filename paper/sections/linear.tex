...
any iterate of the logarithm. Whether $\sup_m$ of the family limit is finite (the defect truly bounded) or
grows like $\ln\ln\ln N$ is open; the exact value is algebraic per $N$ (Proposition~\ref{prop:closed}) with
no apparent universal closed form.
\end{remark}
...
\begin{equation}\label{eq:beta-odd}
  \beta_{\mathrm{odd}}=\sqrt{\tfrac2\pi}\int_0^1\mathbb E_{\Phi,\Psi}
     \sqrt{\frac{\cos^2\Phi}{\sqrt{1-u^2}}+\frac{\cos^2\Psi}{\sqrt{u(2-u)}}}\;\mathrm du=0.92798\ldots,
\end{equation}
an elliptic-integral average ($\Phi,\Psi$ independent uniform) with no apparent elementary closed form
(\texttt{verify\_beta\_odd.py}). Evaluation at $t=N/2$ is rigorous, so $\liminf_N B_N/\sqrt N=c_0/\sqrt2$ and
$\liminf_{\text{odd }N}B_N/\sqrt N\ge\beta_{\mathrm{odd}}$, whence $\limsup_N B_N/\sqrt N\in[\beta_{\mathrm{odd}},1]$.
For the upper side, the triangle inequality (over the Poisson copies) and unitarity give, for \emph{every}
$t$, the two exact bounds $\|K_N(\cdot,t)\|_1\le\sum_{n}|J_n(2t)|$ and $\|K_N(\cdot,t)\|_1\le\sqrt N$; since
$\sum_{n}|J_n(2t)|=(1+o(1))c_0\sqrt{2t}$ (Lemma~\ref{lem:bessel-l1}),
\[
  \|K_N(\cdot,t)\|_1\ \le\ \min\!\Bigl(\textstyle\sum_{n}|J_n(2t)|,\ \sqrt N\Bigr)
                       \ =\ \min\!\bigl((1+o(1))c_0\sqrt{2t},\ \sqrt N\bigr),
\]
which for $t\le N/4$ is $\le(1+o(1))\,c_0/\sqrt2\,\sqrt N$. So any time beating the even value $c_0/\sqrt2$ must lie in
the overlap region $t>N/4$, where the parity of $\mathrm i^{-N}$ decides the cancellation. The remaining
gap---a sharp upper bound there, equivalently that the $\sin^2$ dispersion admits no flat (Gauss-sum)
revival making $|K_N(\cdot,t)|$ uniform---is the one residual analytic question; numerically the sup is
always at $t=N/2$ (odd) or $t=N/4$ (even), so $\limsup_N B_N/\sqrt N=\beta_{\mathrm{odd}}$ is expected.
\end{remark}
