...
$A_N\sim\tfrac1\pi\ln N$ is now proved for \emph{every} $N$ (Theorem~\ref{thm:order}); what is \emph{not}
pinned analytically is the bounded defect $U_N-A_N$ on the non-saturating $N$ (the sub-leading $O(1)$ term),
known only numerically ($A_N/U_N\in[0.84,1]$). (ii)~The saturation locus is settled---no composite $N$
