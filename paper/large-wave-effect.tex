...
by validated interval arithmetic on $[0.937,1]$. The one remaining piece is the rigorous upper bound on the
multi-copy region $t>N/2$ (numerically subdominant---no Gauss-sum revival), which would give
$\limsup_N B_N/\sqrt N=\beta_{\mathrm{odd}}$
(\texttt{verify\_bn\_profile\_rigorous.py}, \texttt{verify\_bn\_parity.py}).
