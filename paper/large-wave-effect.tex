...
\begin{enumerate}
\item Pin down the bounded defect $U_N-A_N$ on the non-saturating $N$. The order
$A_N\sim\tfrac1\pi\ln N$ is now proved for \emph{all} $N$ (Theorem~\ref{thm:order}), and $A_N=U_N$ iff $N$
is prime or $2^m$ (Theorem~\ref{thm:classification}); what remains is the precise size of the $O(1)$ deficit
for composite $N$ (numerically $A_N/U_N\in[0.84,1]$, with $A_N/U_N\to1$ along $N=2p$). Certified per-$N$
brackets follow from the Lasserre moment--SOS hierarchy (Section~\ref{sec:exact}).
\item Pin down the Schr\"odinger constant $B_N/\sqrt N$. The order $B_N=\Theta(\sqrt N)$ is settled, the
$\liminf=c_0/\sqrt2\approx0.861$ is proved (Theorem~\ref{thm:schr-lower}, via the aliasing-free front at
$t=N/4$), and the constant is shown to have \emph{no} limit---it splits by the parity of $N$
(Remark~\ref{rem:bn-parity}). What remains is (a) the exact $\beta_{\mathrm{odd}}=\limsup B_N/\sqrt N$ (a
Bessel-envelope double integral, $\approx0.928$, governed by the $\mathrm i^{-N}$ Gauss-sum phase), and
(b) a matching upper bound strictly below the unitarity value $1$
