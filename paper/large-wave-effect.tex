...
(Theorem~\ref{thm:classification}), and by no composite $N$. A low-mode prefix gives strong evidence that $A_N=\Theta(\ln N)$ for
...
(Theorem~\ref{thm:classification}). A $\Q$-independent low-mode prefix gives strong numerical evidence, but
not yet a proof, that $A_N=\Theta(\ln N)$ on the non-saturating $N$ too (Conjecture~\ref{conj:order}).
...
$L_{\mathrm{pre}}$ is heuristic, not a proven bound. (ii)~The saturation locus is now settled---no
composite $N$ saturates (Theorem~\ref{thm:classification})---but the \emph{value} $A_N$ on the
non-saturating $N$ is only computed per $N$, not bounded in closed form, and its $\Theta(\ln N)$ order
there is Conjecture~\ref{conj:order}. (iii)~The Schr\"odinger order $B_N=\Theta(\sqrt N)$ is proved, but the optimal
...
two ceilings coincide. (iv)~On the Dirichlet segment the saturation classification is now proved
(Proposition~\ref{prop:segclass}); only the exact \emph{deficient} rank $\tfrac12\varphi(4(N+1))$ for
composite $N+1$ (as opposed to the proved upper bound \eqref{eq:segrank}) remains verified
($N\le60$) rather than proved. (v)~Part~II is \emph{computational and exploratory}: its rigorous statements (the Hamiltonian
