...
by validated interval arithmetic on $[0.937,1]$. For the multi-copy region $t>N/2$ the joint
equidistribution of the Debye phases is now \emph{proved} (Theorem~\ref{thm:joint-equi}, a van der Corput
second-derivative bound), as are the variance-floor inequality and the tail limit $\sqrt\pi/2<\beta_{\mathrm
{odd}}$ (Lemma~\ref{lem:varfloor}); two validated, interval-arithmetic-shaped residuals remain---the
uniform-in-$u$ band-edge transfer and the interval enclosure of the profile bound on $(1,6]$---between this
and an unconditional $\limsup_N B_N/\sqrt N=\beta_{\mathrm{odd}}$ (\texttt{verify\_bn\_profile\_rigorous.py},
\texttt{verify\_debye\_equidistribution.py}, \texttt{verify\_bn\_largewave\_tail.py}).
