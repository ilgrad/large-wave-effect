...
% by the named script.  Bibliographic entries marked "% VERIFY" are taken from the
% project notes and must be checked against the primary source before submission.
% ---------------------------------------------------------------------------
...
relation among the frequencies has coefficient-sum divisible by four---an arithmetic criterion met by the
prime and $2^m$ cases (and by no composite $N\le160$). A low-mode prefix gives strong evidence that $A_N=\Theta(\ln N)$ for
...
continuous wave equation the classical bound of Zhukovsky and Lurie gives $|\sigma(x,t)|\le F$~\cite{Zhukovsky}.
...
\emph{finite, discrete spectrum}---finitely many modes whose frequencies are rationally related can be
brought exactly into phase; on the infinite chain the spectrum is continuous, no exact alignment
occurs, and the amplification is absent. It is thus a finite-size spectral resonance rather than a
...
(Theorem~\ref{thm:qrank}); a sharp arithmetic criterion---coefficient-sums divisible by four---decides
exactly when $A_N=U_N$ (Theorem~\ref{thm:ceiling}), while a $\Q$-independent low-mode prefix gives strong
...
asymptotics on the Dirichlet segment in~\cite{MF03} and the Schr\"odinger analogue in~\cite{F23}. Our
contribution is \emph{quantitative and structural}: the explicit ring constant $\tfrac1\pi$ and the
next term $-\tfrac{\pi}{72}N^{-2}$ (Lemma~\ref{lem:asym}); the closed-form subtorus dimension
$\tfrac12\varphi(2N)$ for every $N$ and the cyclotomic independence proof for $N$ prime and $N=2^m$
(Theorems~\ref{thm:qrank},~\ref{thm:prime},~\ref{thm:two}); the exact $\bmod\,4$ criterion for reaching
...
coefficient-sum $\equiv0\ (\mathrm{mod}\ 4)$ (Thm.~\ref{thm:ceiling}); on $N\le160$ the saturating set is
exactly $\{\text{prime}\}\cup\{2^m\}$, of density $0$ (Cor.~\ref{cor:density}).
...
\item[\textbf{T5}] The large wave lives exactly at spectral dimension $d_s=1$ (incl.\ quasi-1D), and is
an infinite-time ceiling: the finite-time amplitude is Diophantine-limited
(\S\ref{sec:dim},~\S\ref{sec:reach}).
...
Section~\ref{sec:dim}, the GLT calculus extends every spectral statement here to variable-coefficient
lattices (non-uniform masses) and anisotropic or graph Laplacians, the natural setting for future work.
...
constant $\lim B_N/\sqrt N\in[0.63,1]$ is open, and the segment constant $C=4/\pi^2$
(Section~\ref{sub:filimonov}) is identified numerically and by a limiting integral rather than a fully
rigorous bound. (iv)~The segment rank \eqref{eq:segrank} is verified to $N\le30$ and conjectured in
...
bound $c\gtrsim0.63$ comes from the unwrapped Bessel front at $t=N/8$, the upper bound $1$ from
unitarity, and a $t$-scan (including the Talbot times $t=\pi a/N$) gives $\sup_N B_N/\sqrt N\approx0.88$--$0.96$;
the optimal constant in $[0.63,1]$ is an almost-periodic / Gauss-sum supremum
