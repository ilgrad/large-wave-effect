...
golden-ratio relation---and the dependent band then carries $\Theta(N)$ relation-locked modes of $\Theta(1)$
total weight whose \emph{signed} net stays $O(1)$. The free sandwich that closes $\omega(m)\le1$ becomes
$\Theta(1)$, so the lemma reduces to a sup-side signed-cancellation estimate (the primal counterpart of
McGehee--Pigno--Smith/Konyagin) on which crude modulus bounds overshoot---structurally the \emph{same}
