...
(Propositions~\ref{prop:breather},~\ref{prop:breather-stab}), and the no-dispersal side of the dynamical
self-trapping threshold, with the exact constant $4$ (Proposition~\ref{prop:threshold}); the $\inf_t$
converse below the bandwidth, the saturation/rogue dynamics, and the route to chaos are simulated, not
proved. (vi)~The
...
(Proposition~\ref{prop:breather-stab}); and one side of the dynamical launch threshold is settled with the
exact constant $4$---no dispersal for $\gamma P>4$ (Proposition~\ref{prop:threshold}). What is open is the
\emph{fully} nonlinear regime---saturation, the correctly stated converse of the threshold (the drop of
$\inf_t\max_j|u_j|^2$ below the bandwidth; on the finite ring $\sup_t$ recurs by Poincar\'e), and the
rogue-wave focusing---where the admissible coupling shrinks with the Diophantine alignment time.
