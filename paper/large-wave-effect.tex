...
$\ell^\infty\!\to\!\ell^\infty$ amplification satisfies $B_N=\Theta(\sqrt N)$; here $\liminf B_N/\sqrt N=
c_0/\sqrt2$ but the constant has \emph{no} limit---it splits by parity, with $\limsup\ge\beta_{\mathrm{odd}}
=0.928\ldots$ (a Bessel-envelope integral with no elementary closed form). For the focusing discrete NLS the
...
(Theorem~\ref{thm:schr-lower}). We claim neither the large-wave phenomenon nor its appearance on the ring
...
$U_N-A_N$ on the non-saturating $N$ is $O(\ln\ln\ln N)$, hence essentially bounded
(Remark~\ref{rem:defect}), but its exact value is algebraic per $N$ with no apparent closed form
...
$B_N=\Theta(\sqrt N)$ the constant has \emph{no} limit---it splits by parity (Remark~\ref{rem:bn-parity}),
with $\liminf_N B_N/\sqrt N=c_0/\sqrt2\approx0.861$ (proved) and $\limsup\ge\beta_{\mathrm{odd}}\approx0.928$;
what is still open is the exact $\beta_{\mathrm{odd}}$ and a matching upper bound below $1$. The segment constant $C=4/\pi^2$ (Section~\ref{sub:filimonov}) is \emph{not} ours: it is the
published Myshkis--Filimonov value~\cite[Thm.~2]{MF03}, which the Schr\"odinger ceiling inherits because the
two ceilings coincide. (iv)~On the Dirichlet segment the rank and saturation are now fully proved
(Theorem~\ref{thm:segrank}); the only remaining gap there is the same as on the ring---the
$\Theta(\ln N)$ \emph{order} of $A_N^{\mathrm{Dir}}$ on the non-saturating $N+1$. (v)~Part~II is \emph{computational and exploratory}: its rigorous statements (the Hamiltonian
...
$\liminf=c_0/\sqrt2\approx0.861$ is proved (Theorem~\ref{thm:schr-lower}), and the constant splits by the
parity of $N$ (Remark~\ref{rem:bn-parity}): the odd-subsequence value $\beta_{\mathrm{odd}}=0.928019303\ldots$
is determined exactly (an elliptic-integral average, shown by a $25$-digit \textsc{pslq} search to have no
elementary closed form), and $\|K_N(\cdot,t)\|_1\le(\beta_{\mathrm{odd}}+o(1))\sqrt N$ is \emph{proved} for
$t\le N/2$. The one residual is the rigorous upper bound on the multi-copy region $t>N/2$ (numerically
subdominant---no Gauss-sum revival), which would give $\limsup_N B_N/\sqrt N=\beta_{\mathrm{odd}}$
...
(Proposition~\ref{prop:dnls-mi}); breathers \emph{exist} on the ring by a finite-dimensional
